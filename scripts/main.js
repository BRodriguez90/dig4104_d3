...
$(function () {
    ...
    $('#draw').click(function () {
        ...
        $('svg').show();
        var data = [{ key: 'HTML', lvl: 9, skill: 'Expert' }, { key: 'CSS', lvl: 8, skill: 'Pro' }, { key: 'JavaScript', lvl: 6, skill: 'Now we\'re talking' }, { key: 'jQuery', lvl: 5, skill: 'Not too shabby' }, { key: 'PHP', lvl: 4, skill: 'Not bad' }, { key: 'mySQL', lvl: 3, skill: 'Noobie' }, { key: 'Node.js', lvl: 2, skill: 'Needs Work' }, { key: 'd3', lvl: 1, skill: 'meh' }, { key: 'Angular2', lvl: 2, skill: 'Needs Work' }, { key: '', lvl: 0, skill: '' }, { key: 'Photoshop', lvl: 8, skill: 'Pro' }, { key: 'Illustrator', lvl: 5, skill: 'Now we\'re talking' }, { key: 'Premiere', lvl: 4, skill: 'Not bad' }];
        ...
        .domain([0, maxData]).range(['#4da6ff', '#003366']);
        var y_names = d3.scale.ordinal().domain([0, maxData]).range([data.skill, data.skill]);
        //Axis
        ...
        // Tip when hovered
        var tip = d3.tip().attr('class', 'd3-tip').offset([-15, 0]).html(function (d) {
        ...
        chart.append('g').attr('class', 'x axis').attr('transform', 'translate(0,' + height + ')').call(xAxis).selectAll('text').attr('y', 7).attr('x', 7).attr('dy', '.35em').attr('transform', 'rotate(27)').style('text-anchor', 'start');;

        chart.append('g').attr('class', 'y axis').call(yAxis).append('text').attr('transform', 'rotate(270)').attr('y', 6).attr('dy', '.71em').attr('font-size', '13px').style('text-anchor', 'end');
        ...
        chart.selectAll('.bar').data(data).enter().append('rect').attr('class', 'bar').attr('x', function (d) {
            ...
            return height - y(d.lvl);
        }).attr('width', x.rangeBand()).attr('width', '80px').on('mouseover', tip.show).on('mouseout', tip.hide).style('stroke', '#fff').style('stroke-width', '2').style('opacity', '0.9').style('fill', 'transparent')
        //.style('fill', '#fcb052')
        //.style('fill','#4e6096')
        //.style("fill", function(d) { return color(d.val);})
        .style('stroke-dasharray', '1500').style('stroke-dashoffset', '1600').style('animation', 'outline 5s linear').style('animation-fill-mode', 'forwards');

        var t = d3.transition().duration(1000).ease(d3.easeLinear);

        d3.selectAll('rect').transition(t).style('fill', function (d) {
            return color(d.lvl);
        }).delay(3450).style('stroke', '#000');
    });
});
